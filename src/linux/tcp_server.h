...
public:
    ...
    bool SendWithHeader(ConnectionId cid, const void *hdr, size_t hdr_len, const void *data,
                        size_t data_len);
    bool CloseConnection(ConnectionId cid);

    // internal::IAcceptor impl
    void OnNewConnection(int sock_fd, int listen_port,
...
private:
    ...
    using ConnectionData = std::pair<std::shared_ptr<Connection>, TimeoutRecord::iterator>;
