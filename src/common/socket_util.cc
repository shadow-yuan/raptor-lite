    ...
    if (addr->sa_family == AF_INET6) {
        ...
        if (memcmp(addr6->sin6_addr.s6_addr, kV4MappedPrefix, sizeof(kV4MappedPrefix)) == 0) {
            if (resolved_addr4_out != nullptr) {
                ...
                resolved_addr4_out->len = static_cast<socklen_t>(sizeof(raptor_sockaddr_in));
            }
            return 1;
        }
    }
...
void raptor_sockaddr_make_wildcard4(int port, raptor_resolved_address *resolved_wild_out) {
    raptor_sockaddr_in *wild_out = reinterpret_cast<raptor_sockaddr_in *>(resolved_wild_out->addr);
...
int raptor_sockaddr_set_port(const raptor_resolved_address *resolved_addr, int port) {
    ...
    switch (addr->sa_family) {
    case AF_INET:
        ((raptor_sockaddr_in *)addr)->sin_port = htons(static_cast<uint16_t>(port));
        return 1;
    case AF_INET6:
        ((raptor_sockaddr_in6 *)addr)->sin6_port = htons(static_cast<uint16_t>(port));
        return 1;
    default:
...
int raptor_sockaddr_get_ip(const raptor_resolved_address *resolved_addr, char *dst, size_t size) {
    ...
    if (ip != nullptr &&
        ...
        if (sin6_scope_id != 0) {
            ...
            ret = static_cast<int>(strlen(ntop_buf));
