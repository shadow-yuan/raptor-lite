...
void SetConsoleColor(FILE *f, Color foreground, Color background) {
    ...
    WORD attr = ForegroundColorTable[static_cast<int>(foreground)] +
                BackgroundColorTable[static_cast<int>(background)];
...
void FprintColorTextLine(FILE *f, Color foreground, Color background, const char *text) {
    SetConsoleColor(f, foreground, background);
    fprintf(f, "%s", text);
    ResetConsoleColor(f);
    fprintf(f, "\n");
}
...
namespace raptor {
void SetConsoleColor(FILE *f, Color foreground, Color background) {
    fprintf(f, "%s%s", ForegroundColorTable[static_cast<int>(foreground)],
            BackgroundColorTable[static_cast<int>(background)]);
}
...
void FprintColorTextLine(FILE *f, Color foreground, Color background, const char *text) {
    SetConsoleColor(f, foreground, background);
    fprintf(f, "%s%s%s%s\n", ForegroundColorTable[static_cast<int>(foreground)],
            BackgroundColorTable[static_cast<int>(background)], text, colorReset);
    ResetConsoleColor(f);
}
}  // namespace raptor
